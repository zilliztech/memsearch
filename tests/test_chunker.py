...
def test_mixed_double_colons_do_not_act_as_sentence_boundaries() -> None:
    """Double-colon namespace fragments should not be treated as sentence boundaries."""
    text = ("请检查 memsearch::chunker 和 crate::parser::split 这些片段是否正常" * 4)

    chunks = chunk_markdown(text, source="double-colons.md", max_chunk_size=34)
...
def test_mixed_percent_signs_do_not_act_as_sentence_boundaries() -> None:
    """Percent-sign fragments should not be treated as sentence boundaries."""
    text = ("请检查 %APPDATA% 和 %(message)s 这类片段是否正常" * 4)

    chunks = chunk_markdown(text, source="percent-signs.md", max_chunk_size=30)
...
def test_mixed_hash_signs_do_not_act_as_sentence_boundaries() -> None:
    """Hash-sign fragments should not be treated as sentence boundaries."""
    text = ("请检查 #ops-alert 和 issue #102 以及 PR #388 这类片段是否正常" * 4)

    chunks = chunk_markdown(text, source="hash-signs.md", max_chunk_size=32)
...
def test_mixed_tildes_do_not_act_as_sentence_boundaries() -> None:
    """Tilde fragments should not be treated as sentence boundaries."""
    text = ("请检查 ~/workspace 和 version~beta 这类片段是否正常" * 4)

    chunks = chunk_markdown(text, source="tildes.md", max_chunk_size=30)
