...
def test_cjk_colon_does_not_act_as_sentence_boundary() -> None:
    ...
    colon = "\uff1a"
    text = (f"处理步骤如下{colon}继续检查日志输出{colon}继续确认索引状态{colon}") * 4

    chunks = chunk_markdown(text, source="colon.md", max_chunk_size=22)

    assert len(chunks) > 1
    assert all(len(chunk.content) <= 22 for chunk in chunks)
    assert not all(chunk.content.endswith(colon) for chunk in chunks[:-1])
...
def test_cjk_parentheses_do_not_act_as_sentence_boundaries() -> None:
    ...
    right_paren = "\uff09"
    text = (
        f"请检查缓存命中{left_paren}重点关注热数据{right_paren}再检查索引状态{left_paren}确认已完成{right_paren}"
    ) * 4
    ...
    assert not all(chunk.content.endswith((left_paren, right_paren)) for chunk in chunks[:-1])
