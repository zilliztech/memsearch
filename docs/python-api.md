...
The [Claude Code plugin](platforms/claude-code/index.md) uses per-project isolation -- each project automatically gets its own Milvus collection (e.g. `ms_my_app_a1b2c3d4`) derived from the project path, so searches never leak across projects. But the underlying memsearch library has no such constraint -- a consumer chat app can instantiate one `MemSearch` per user and get clean isolation.
