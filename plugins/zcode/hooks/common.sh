#!/usr/bin/env bash
# Shared setup for memsearch ZCode hooks.
...
# same collection even when ZCode is launched from a subdirectory.
if [ -n "${MEMSEARCH_PROJECT_DIR:-}" ] && [ -d "${MEMSEARCH_PROJECT_DIR:-}" ]; then
  PROJECT_DIR="$MEMSEARCH_PROJECT_DIR"
else
  HOOK_CWD="${ZCODE_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-$(pwd)}}"
...
# --- ZCode session DB ---
#
# ZCode stores conversations in a SQLite database, not a JSONL transcript file.
# The default path is ~/.zcode/cli/db/db.sqlite; ZCODE_DB_PATH overrides it.
ZCODE_DB_PATH="${ZCODE_DB_PATH:-$HOME/.zcode/cli/db/db.sqlite}"

# --- Session ID ---
#
# ZCode injects CLAUDE_SESSION_ID as a template variable / environment variable.
ZCODE_SESSION_ID="${CLAUDE_SESSION_ID:-${ZCODE_SESSION_ID:-}}"
...
# --- Index state warning (shared diagnostic) ---
