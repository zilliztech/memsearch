...
**Drill into the original before drafting — and read the tool calls.** The journal bullets are a lossy summary; writing from them alone produces plausible-but-wrong steps. Each entry carries an anchor naming the original conversation file and its path (here: `rollout:<file>.jsonl` (Codex uses `rollout:` and usually has no `turn:`)). The reliable way to recover exact commands, flags, paths, and outputs is to **open that JSONL file directly** and read the `function_call` / `function_call_output` entries for the relevant session/turn — that is where the executed commands actually live. The plugin's recall/transcript helper can reproduce the conversation text for context, but it **omits tool calls and their output**, so never reconstruct a command from it. If you cannot confirm a detail, keep that step general or omit it — never fabricate.
